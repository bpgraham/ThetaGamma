...
: V3 allows spec of number of bursts (BPG 12-10-12)
: V4 correctly specifies burst start and end times irrespective of first spike time
:    previous versions mark start of first burst from first spike time (BPG 28-8-15)

NEURON	{ 
  ARTIFICIAL_CELL BurstStim4
...
PARAMETER {
	...
	number	= 10 <0,1e9>	: total number of bursts
...
INITIAL {
	...
	if (start >= 0 && number > 0) {
		...
		: }
		: net_send(event, 3)
		net_send(start, 3)	: start of first burst
	}
}	
...
NET_RECEIVE (w) {
	...
	if (flag == 3) { : from INITIAL
		if (on == -1) { : but ignore if turned off by external event
			init_sequence(t)
			: net_send(0, 1)
			net_send(0, 2)	: to start first burst
		}
	}
	if (flag == 2) { : burst control
		if (on == 0 && iburst < number) { : start burst
			on = 1
			iburst = iburst + 1
			next_invl()
			event = event - interval*(1. - noise)
			net_send(event, 1)	: first spike in burst
			net_send(burstlen, 2)	: to terminate burst
		}
		else { : end burst
			on = 0
			net_send(burstint, 2)	: to start next burst
		}
	}
	if (flag == 1 && on == 1) {
		net_event(t)
